...
Preprocess::process_cut_frame_pcl2(const sensor_msgs::PointCloud2::ConstPtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out,
    ...
    if (lidar_type == VELO) {
        ...
        // Normalize per-point time if driver provides absolute time or microseconds.
        // Expectation: per-point time within a frame is ~0..0.1s.
        float t0_velo = plsize > 0 ? pl_orig.points[0].time : 0.0f;
        bool need_time_normalize = false;
        bool time_is_micro = false;
        if (plsize > 1) {
            float t_last = pl_orig.points[plsize - 1].time;
            float span = t_last - t0_velo;
            if (t_last > 1.0f || span > 1.0f) need_time_normalize = true; // absolute seconds or large span
            if (span > 1e3f) time_is_micro = true; // likely microseconds scale
        }
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            float t_pt = pl_orig.points[i].time;
            double t_sec;
            if (need_time_normalize) {
                double dt = static_cast<double>(t_pt - t0_velo);
                if (time_is_micro) dt *= 1e-6; // us -> s
                if (dt < 0) dt = 0.0;
                t_sec = dt;
            } else {
                t_sec = static_cast<double>(t_pt);
            }
            added_pt.curvature = t_sec * 1000.0;  // ms
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    if (plsize > 1) {
        float t_last = pl_orig.points[plsize - 1].time;
        float span = t_last - t0_velo;
        if (t_last > 1.0f || span > 1.0f) need_time_normalize = true;
        if (span > 1e3f) time_is_micro = true; // likely microseconds
    }
    ...
    if (feature_enabled) {
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.curvature = t_sec * 1000.0; // unit: ms
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            // Use normalized per-scan time if needed
            {
                float t_pt = pl_orig.points[i].time;
                double t_sec;
                if (need_time_normalize) {
