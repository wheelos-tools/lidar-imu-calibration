...
void LI_Init::fout_before_filter() {
    if (IMU_state_group.size() < 1 || Lidar_state_group.size() < 1) return;
...
void LI_Init::downsample_interpolate_IMU(const double &move_start_time) {
    if (IMU_state_group_ALL.empty() || Lidar_state_group.empty()) return;
    ...
    if (IMU_state_group_ALL.size() < 2 || Lidar_state_group.size() < 2) return;
    ...
    for (int i = 0; i < (int)Lidar_state_group.size(); i++) {
        for (int j = 1; j < (int)IMU_state_group_ALL.size(); j++) {
...
void LI_Init::IMU_time_compensate(const double &lag_time, const bool &is_discard) {
    if (IMU_state_group.size() < 2 || Lidar_state_group.size() < 1) return;
    if (is_discard) {
        //Discard first 10 Lidar estimations and corresponding IMU measurements due to long time interval
        int keep = std::min(10, (int)std::min(IMU_state_group.size(), Lidar_state_group.size()) - 1);
        for (int i = 0; i < keep; i++) { Lidar_state_group.pop_front(); IMU_state_group.pop_front(); }
    }
    ...
    if (IMU_state_group.empty() || Lidar_state_group.empty()) return;
    while (!Lidar_state_group.empty() && !IMU_state_group.empty() && Lidar_state_group.front().timeStamp < IMU_state_group.front().timeStamp)
        Lidar_state_group.pop_front();
    if (IMU_state_group.size() < 2 || Lidar_state_group.empty()) return;
    while (!Lidar_state_group.empty() && IMU_state_group.size() > 1 && Lidar_state_group.front().timeStamp > IMU_state_group[1].timeStamp)
...
void LI_Init::cut_sequence_tail() {
    if (IMU_state_group.size() < 3 || Lidar_state_group.size() < 3) return;
    int cut = std::min(20, (int)std::min(Lidar_state_group.size(), IMU_state_group.size()) - 2);
    for (int i = 0; i < cut; ++i) { Lidar_state_group.pop_back(); IMU_state_group.pop_back(); }
    while (!Lidar_state_group.empty() && !IMU_state_group.empty() && Lidar_state_group.front().timeStamp < IMU_state_group.front().timeStamp)
        Lidar_state_group.pop_front();
    while (IMU_state_group.size() > 1 && !Lidar_state_group.empty() && Lidar_state_group.front().timeStamp > IMU_state_group[1].timeStamp)
        IMU_state_group.pop_front();
...
void LI_Init::acc_interpolate() {
    ...
    if (IMU_state_group.size() < 3 || Lidar_state_group.size() < 3) return;
    for (int i = 1; i < (int)Lidar_state_group.size() - 1; i++) {
...
void LI_Init::Butter_filt(const deque<CalibState> &signal_in, deque<CalibState> &signal_out) {
    ...
    if ((int)signal_in.size() <= 2 * butter.extend_num + butter.Coeff_size) {
        // not enough samples for filtering; pass-through
        signal_out.assign(signal_in.begin(), signal_in.end());
        return;
    }
...
void LI_Init::zero_phase_filt(const deque<CalibState> &signal_in, deque<CalibState> &signal_out) {
    if (signal_in.empty()) { signal_out.clear(); return; }
...
bool LI_Init::data_sufficiency_assess(MatrixXd &Jacobian_rot, int &frame_num, V3D &lidar_omg, int &orig_odom_freq,
    ...
    // Ensure Jacobian_rot has enough rows: we write a 3x3 block at row 3*frame_num
    int write_row = 3 * frame_num;
    int need_rows = write_row + 3;
    if (need_rows > Jacobian_rot.rows()) {
        int old_rows = Jacobian_rot.rows();
        // grow with margin to avoid frequent reallocations
        int new_rows = need_rows + 3000;
        Jacobian_rot.conservativeResize(new_rows, Eigen::NoChange);
        // zero the newly added rows
        Jacobian_rot.block(old_rows, 0, new_rows - old_rows, Jacobian_rot.cols()).setZero();
    }
    M3D lidar_omg_skew;
...
void LI_Init::LI_Initialization(int &orig_odom_freq, int &cut_frame_num, double &timediff_imu_wrt_lidar,
    ...
    auto print_range = [](const char *tag, const deque<CalibState> &seq) {
        if (seq.empty()) {
            printf(BOLDCYAN "[InitDBG] %s range: EMPTY\n" RESET, tag);
            return;
        }
        double tmin = seq.front().timeStamp, tmax = seq.front().timeStamp;
        for (const auto &s : seq) {
            if (s.timeStamp < tmin) tmin = s.timeStamp;
            if (s.timeStamp > tmax) tmax = s.timeStamp;
        }
        printf(BOLDCYAN "[InitDBG] %s range: [%.6f, %.6f], dur=%.6f s, size=%zu\n" RESET,
               tag, tmin, tmax, tmax - tmin, seq.size());
    };

    printf(BOLDCYAN "[InitDBG] Enter LI_Initialization. move_start_time=%.6f, IMU_ALL=%zu, Lidar=%zu\n" RESET,
           move_start_time, IMU_state_group_ALL.size(), Lidar_state_group.size());
    print_range("IMU_ALL", IMU_state_group_ALL);
    print_range("Lidar  ", Lidar_state_group);

    downsample_interpolate_IMU(move_start_time);
    printf(BOLDCYAN "[InitDBG] after downsample_interpolate_IMU: IMU_ALL=%zu, IMU=%zu, Lidar=%zu\n" RESET,
           IMU_state_group_ALL.size(), IMU_state_group.size(), Lidar_state_group.size());
    print_range("IMU    ", IMU_state_group);
    print_range("Lidar  ", Lidar_state_group);

    fout_before_filter();
    printf(BOLDCYAN "[InitDBG] after fout_before_filter: IMU=%zu, Lidar=%zu\n" RESET,
           IMU_state_group.size(), Lidar_state_group.size());

    IMU_time_compensate(0.0, true);
    printf(BOLDCYAN "[InitDBG] after IMU_time_compensate(0): IMU=%zu, Lidar=%zu\n" RESET,
           IMU_state_group.size(), Lidar_state_group.size());
    print_range("IMU    ", IMU_state_group);
    print_range("Lidar  ", Lidar_state_group);
    ...
    printf(BOLDCYAN "[InitDBG] after zero_phase_filt(IMU): in=%zu, out=%zu\n" RESET,
           get_IMU_state().size(), IMU_after_zero_phase.size());
    normalize_acc(IMU_after_zero_phase);
    zero_phase_filt(get_Lidar_state(), Lidar_after_zero_phase);
    printf(BOLDCYAN "[InitDBG] after zero_phase_filt(Lidar): in=%zu, out=%zu\n" RESET,
           get_Lidar_state().size(), Lidar_after_zero_phase.size());
    set_IMU_state(IMU_after_zero_phase);
    set_Lidar_state(Lidar_after_zero_phase);
    printf(BOLDCYAN "[InitDBG] after set_states: IMU=%zu, Lidar=%zu\n" RESET,
           IMU_state_group.size(), Lidar_state_group.size());
    print_range("IMU    ", IMU_state_group);
    print_range("Lidar  ", Lidar_state_group);
    if (IMU_state_group.size() < 20 || Lidar_state_group.size() < 20) {
        printf(RED "[Initialization] Abort: insufficient data after filtering (IMU:%zu, LiDAR:%zu). Keep collecting.\n" RESET,
               IMU_state_group.size(), Lidar_state_group.size());
        return;
    }
    cut_sequence_tail();
    printf(BOLDCYAN "[InitDBG] after cut_sequence_tail: IMU=%zu, Lidar=%zu\n" RESET,
           IMU_state_group.size(), Lidar_state_group.size());
    print_range("IMU    ", IMU_state_group);
    print_range("Lidar  ", Lidar_state_group);
    if (IMU_state_group.size() < 10 || Lidar_state_group.size() < 10) {
        printf(RED "[Initialization] Abort: insufficient data after trimming (IMU:%zu, LiDAR:%zu).\n" RESET,
               IMU_state_group.size(), Lidar_state_group.size());
        return;
    }

    xcorr_temporal_init(orig_odom_freq * cut_frame_num);
    printf(BOLDCYAN "[InitDBG] after xcorr_temporal_init: lag1=%.6f, lag_idx=%d\n" RESET,
           time_lag_1, lag_IMU_wtr_Lidar);
    IMU_time_compensate(get_lag_time_1(), false);
    printf(BOLDCYAN "[InitDBG] after IMU_time_compensate(lag1): IMU=%zu, Lidar=%zu\n" RESET,
           IMU_state_group.size(), Lidar_state_group.size());
    print_range("IMU    ", IMU_state_group);
    print_range("Lidar  ", Lidar_state_group);

    central_diff();
    printf(BOLDCYAN "[InitDBG] after central_diff: IMU=%zu, Lidar=%zu\n" RESET,
           IMU_state_group.size(), Lidar_state_group.size());
    if (IMU_state_group.size() < 10 || Lidar_state_group.size() < 10) {
        printf(RED "[Initialization] Abort: insufficient data for optimization (IMU:%zu, LiDAR:%zu).\n" RESET,
               IMU_state_group.size(), Lidar_state_group.size());
        return;
    }
    ...
    printf(BOLDCYAN "[InitDBG] call solve_Rotation_only\n" RESET);
    solve_Rotation_only();
    printf(BOLDCYAN "[InitDBG] after solve_Rotation_only\n" RESET);

    printf(BOLDCYAN "[InitDBG] call solve_Rot_bias_gyro\n" RESET);
    solve_Rot_bias_gyro(timediff_imu_wrt_lidar);
    printf(BOLDCYAN "[InitDBG] after solve_Rot_bias_gyro: time_lag_1=%.6f time_lag_2=%.6f total=%.6f\n" RESET,
           time_lag_1, time_lag_2, time_delay_IMU_wtr_Lidar + timediff_imu_wrt_lidar);

    printf(BOLDCYAN "[InitDBG] call acc_interpolate\n" RESET);
    acc_interpolate();
    printf(BOLDCYAN "[InitDBG] after acc_interpolate\n" RESET);

    printf(BOLDCYAN "[InitDBG] call solve_trans_biasacc_grav\n" RESET);
    solve_trans_biasacc_grav();
    printf(BOLDCYAN "[InitDBG] after solve_trans_biasacc_grav\n" RESET);
